...
def visualize_bayesian_weights(bayesian_layer, num_samples=1000, num_neurons_to_plot=20):
    ...
    # Generating samples
    samples = np.random.normal(weight_mu, np.sqrt(variance), size=(num_samples, *weight_mu.shape))
    ...
    for i in range(min(num_neurons_to_plot, weight_mu.shape[0])):
        ...
        sns.histplot(samples[:, i, :].flatten(), bins=30, kde=True, color="skyblue", ax=ax)
