...
from pplib.nas.mutables import OneShotChoiceRoute
...
class TestNB301(TestCase):
    ...
    def test_osnb301_choiceroute(self):
        ...
        normal_list = []
        reduce_list = []
        for idx, choices in osm.random_subnet.items():
            # print(idx, choice)
            if isinstance(choices, list):
                ...
                for choice in choices:
                    if 'normal' in choice:
                        ...
                        normal_list.append((current_key, idx_of_op))
                        ...
                        idx_of_op = int(choice[-1])
                        current_key = list(c_route._edges.keys())[idx_of_op]
                        reduce_list.append((current_key, idx_of_op))
        genotype = Genotype(
            normal=normal_list,
