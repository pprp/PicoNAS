...
class TransNASBenchAPI(object):
    ...
    def __init__(self, database_path, verbose=False):
        ...
        self.metrics_dict = self.database[
            'metrics_dict']  # {task : metrics_list}
        ...
        self.all_arch_dict = {
            k: list(self.database['data'][k].keys())
            for k in self.search_spaces
        }
    ...
    def __repr__(self):
        return '{name}({total} architectures/{task} tasks)'.format(
            name=self.__class__.__name__,
            total=len(self),
            task=len(self.task_list))
    ...
    def get_model_info(self, arch, task, info):
        assert info in self.info_names, (f'info {info} is not available! '
                                         'Must in {self.info_names}!')
        arch_result = self.get_arch_result(arch)
    ...
    def get_single_metric(self, arch, task, metric, mode='best', xseed=None):
        ...
        assert metric in self.metrics_dict[task], (
            f'metric {metric} is not available for task {task}! '
            'Must in {self.metrics_dict[task]}!')
        arch_result = self.get_arch_result(arch)
        ...
        if isinstance(mode, str):
            ...
            assert mode < len(
                metric_list
            ), f'get_metric() int mode must < total epoch {len(metric_list)} for task {task}!'
            return metric_list[mode]
        else:
            raise ValueError(
                f"get_metric() mode must be 'final', 'best', 'list' or epoch_number"
            )
    ...
    def get_epoch_status(self, arch, task, epoch, xseed=None):
        ...
        ep_status = {
            'epoch':
            epoch if epoch >= 0 else self.get_total_epochs(arch, task) + epoch
        }
        ep_status = {
            **ep_status,
            **{k: exp_dict[k][epoch]
               for k in self.metrics_dict[task]},
        }
        return ep_status
    ...
    def get_best_epoch_status(self, arch, task, metric):
        ...
        """
        assert (
            metric in self.metrics_dict[task]
    ...
    def get_best_archs(self, task, metric, search_space, topk=1):
        ...
        tuple_list = list(
            map(
                lambda arch: (
                    self.get_single_metric(arch, task, metric, mode='best'),
                    arch,
                ),
                arch_list,
            ))
    ...
    def _gen_all_data(self, database, arch2space):
        ...
        for idx, (arch, space) in enumerate(arch2space.items()):
            data[space][arch] = ArchResult(idx, arch,
                                           database['data'][space][arch])
        return data
...
class ArchResult(object):
    ...
    def __repr__(self):
        return '{name}(arch-index={index}, arch={arch}, {num} tasks)'.format(
            name=self.__class__.__name__,
