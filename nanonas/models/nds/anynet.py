...
from .net import (complexity_batchnorm2d, complexity_conv2d,
                  complexity_maxpool2d, init_weights)
...
class VanillaBlock(nn.Module):
    ...
    def __init__(self, w_in, w_out, stride, bm=None, gw=None, se_r=None):
        ...
        self.a = nn.Conv2d(
            w_in, w_out, 3, stride=stride, padding=1, bias=False)
...
class BottleneckTransform(nn.Module):
    ...
    def __init__(self, w_in, w_out, stride, bm, gw, se_r):
        ...
        self.a_relu = nn.ReLU(inplace=True)
        self.b = nn.Conv2d(
            w_b, w_b, 3, stride=stride, padding=1, groups=g, bias=False)
...
class AnyStage(nn.Module):
    ...
    def __init__(self, w_in, w_out, stride, d, block_fun, bm, gw, se_r):
        ...
        for i in range(d):
            ...
            self.add_module(name,
                            block_fun(b_w_in, w_out, b_stride, bm, gw, se_r))
    ...
    def complexity(cx, w_in, w_out, stride, d, block_fun, bm, gw, se_r):
        for i in range(d):
            b_stride = stride if i == 0 else 1
            b_w_in = w_in if i == 0 else w_out
            cx = block_fun.complexity(cx, b_w_in, w_out, b_stride, bm, gw,
                                      se_r)
        return cx
...
class AnyNet(nn.Module):
    ...
    def _construct(self, stem_type, stem_w, block_type, ds, ws, ss, bms, gws,
                   se_r, nc):
        ...
        for i, (d, w, s, bm, gw) in enumerate(stage_params):
            name = 's{}'.format(i + 1)
            self.add_module(name,
                            AnyStage(prev_w, w, s, d, block_fun, bm, gw, se_r))
            prev_w = w
