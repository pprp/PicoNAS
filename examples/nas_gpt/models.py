...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, max_len=96):
    ...
    def create_positinal_encoding(self, max_len, d_model):
        ...
        for pos in range(max_len):  # for each position of the word
            for i in range(0, d_model,
                           2):  # for each dimension of the each position
    ...
    def forward(self, encoded_words):
        ...
        embedding += self.pe[:, :embedding.size(
            1
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        key = key.view(key.shape[0], -1, self.heads,
                       self.d_k).permute(0, 2, 1, 3)
        value = value.view(value.shape[0], -1, self.heads,
                           self.d_k).permute(0, 2, 1, 3)
        ...
        scores = scores.masked_fill(mask == 0,
                                    -1e9)  # (batch_size, h, max_len, max_len)
        weights = F.softmax(
            scores, dim=-1)  # (batch_size, h, max_len, max_len)
        weights = self.dropout(weights)
        # (batch_size, h, max_len, max_len) matmul (batch_size, h, max_len, d_k) --> (batch_size, h, max_len, d_k)
        context = torch.matmul(weights, value)
        # (batch_size, h, max_len, d_k) --> (batch_size, max_len, h, d_k) --> (batch_size, max_len, h * d_k)
        context = context.permute(0, 2, 1,
                                  3).contiguous().view(context.shape[0], -1,
                                                       self.heads * self.d_k)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, embeddings, encoded, src_mask, target_mask):
        query = self.dropout(
            self.self_multihead(embeddings, embeddings, embeddings,
                                target_mask))
...
class Transformer(nn.Module):
    ...
    def decode(self, target_words, target_mask, src_embeddings, src_mask):
        ...
        for layer in self.decoder:
            tgt_embeddings = layer(tgt_embeddings, src_embeddings, src_mask,
                                   target_mask)
