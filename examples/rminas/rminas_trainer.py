...
from xnas.core.builder import optimizer_builder, setup_env, space_builder
...
from pplib.core.losses.rmi_loss import RMI_loss
...
def main():
    ...
    assert cfg.SPACE.NAME in [
        'infer_nb201', 'infer_darts', 'nasbenchmacro', 'proxyless'
    ]
    assert cfg.LOADER.DATASET in [
        'cifar10', 'cifar100', 'imagenet', 'imagenet16_120'
    ], 'dataset error'

    if cfg.LOADER.DATASET == 'cifar10':
        from xnas.algorithms.RMINAS.teacher_model.resnet20_cifar10.resnet import \
            resnet20
        checkpoint_res = torch.load(
            'xnas/algorithms/RMINAS/teacher_model/resnet20_cifar10/resnet20.th'
        )
        ...
        from xnas.algorithms.RMINAS.teacher_model.resnet101_cifar100.resnet import \
            resnet101
        network = resnet101()
        network.load_state_dict(
            torch.load(
                'xnas/algorithms/RMINAS/teacher_model/resnet101_cifar100/resnet101.pth'
            ))
        ...
        logger.warning(
            'Only partial tests have been conducted, please use with caution.')
    ...
    more_data_X, more_data_y = get_random_data(cfg.LOADER.BATCH_SIZE,
                                               cfg.LOADER.DATASET)
    with torch.no_grad():
    ...
    RFS = RF_suggest(
        ...
        logger=logger,
        api=api,
        thres_rate=cfg.RMINAS.RF_THRESRATE,
        seed=cfg.RNG_SEED)
    ...
    def train_arch(modelinfo):
        ...
        if cfg.SPACE.NAME == 'infer_nb201':
            ...
            model = space_builder(
                stage_width_list=[16, 24, 40, 80, 96, 192, 320],
                depth_param=modelinfo[:6],
                ks=modelinfo[6:27][modelinfo[6:27] > 0],
    ...
    def train_procedure(sample):
        if cfg.SPACE.NAME == 'infer_nb201':
            ...
            sample_geno = geno_from_alpha(
                sampling.darts_sug2alpha(sample))  # type=Genotype
            ...
            mixed_loss = np.inf if np.isnan(mixed_loss) else mixed_loss
            trained_loss.append(mixed_loss)
            RFS.trained_arch.append({
                'arch': sample,
                'loss': mixed_loss,
                'gt': api[sample_geno]['mean_acc'],
    ...
    if sampling_cnt >= cfg.RMINAS.RF_SUCC:
        logger.info('successfully sampling good archs for {} times'.format(
            sampling_cnt))
    else:
        logger.info('failed sampling good archs for only {} times'.format(
            sampling_cnt))
    logger.info('RF sampling time cost:{}'.format(
        str(time.time() - sampling_time)))

    # ====== Evaluation ======
    logger.info('Total time cost: {}'.format(str(time.time() - start_time)))
    logger.info('Actual training times: {}'.format(
        len(RFS.trained_arch_index)))
    if cfg.SPACE.NAME == 'infer_nb201':
        logger.info('Searched architecture:\n{}'.format(
            str(RFS.optimal_arch(method='sum', top=50))))
        logger.info('Searched architecture:\n{}'.format(
            str(RFS.optimal_arch(method='greedy', top=50))))
    elif cfg.SPACE.NAME == 'infer_darts':
