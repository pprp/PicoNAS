...
class GDNASEvaluator(Evaluator):
    ...
    __known_metrics_in_GDNAS = ('FLOPS', 'latency', 'runtime', 'size_MB',
    ...
    def __init__(self,
        ...
        assert GDNAS_type in self.__known_metrics_in_GDNAS, \
            f'Not support type {GDNAS_type}.'
        ...
        self.GDNAS_api = GDNAS_bench.Benchmark(
    ...
    def convert_genostr2genoobj(self, genotype: str):
        from examples.GDNAS_nb201.nasbench201.genos import Structure
        structure = Structure.str2structure(genotype)
        return structure.tolist(remove_str='')[0]

    def query_GDNAS_result(self, config: dict):
        ...
        """
        results = self.GDNAS_api(config, nepochs=200)
        return results[200][self.GDNAS_type]
    ...
    def compute_rank_consistency(self, dataloader,
        ...
        for _ in range(num_sample):
            cfg = self.GDNAS_api.sample_config()
            results = self.query_GDNAS_result(cfg)  # type is eval_acc1es
    ...
    def compute_rank_by_predictive(self,
        ...
        for _ in tqdm(range(num_sample)):
            ...
            results = self.query_GDNAS_result(genotype)  # type is eval_acc1es
            true_indicator_list.append(results)
            ...
            results = self.query_GDNAS_result(genotype, cost_key='flops')
            ...
            genotype = self.generate_genotype(random_subnet_dict,
                                              self.trainer.mutator)
            results2 = self.query_GDNAS_result(genotype)  # type is eval_acc1es
