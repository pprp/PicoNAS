...
class ShuffleXModule(nn.Module):
    def __init__(self, inc, ouc, stride, supernet=True):
        ...
        self.cb_main = nn.Sequential(
            ...
            nn.BatchNorm2d(self.inc, affine=self.affine),
            # pw
            nn.Conv2d(self.inc, self.midc, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(self.midc, affine=self.affine),
            nn.ReLU(inplace=True),
            # dw
            ...
            nn.BatchNorm2d(self.midc, affine=self.affine),
            # pw
            nn.Conv2d(self.midc, self.midc, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(self.midc, affine=self.affine),
            nn.ReLU(inplace=True),
            # dw
            nn.Conv2d(self.midc, self.midc, kernel_size=3, stride=1,
            ...
            nn.BatchNorm2d(self.midc, affine=self.affine),
            # pw
            nn.Conv2d(self.midc, self.ouc, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(self.ouc, affine=self.affine),
            nn.ReLU(inplace=True)
        )
        if stride == 2:
            self.cb_proj = nn.Sequential(
                ...
                nn.BatchNorm2d(self.inc, affine=self.affine),
                # pw
                nn.Conv2d(self.inc, self.inc, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.inc, affine=self.affine),
                nn.ReLU(inplace=True)
            )
