...
class DynamicLinear(DynamicMixin, Linear):
    ...
    def __init__(
        ...
        if isinstance(in_features, MutableValue):
            ...
            raise f'The type of in_features {type(in_features)} is not' 'supported, Only int or MutableValue is supported currently.'
        ...
        if isinstance(out_features, MutableValue):
            ...
            raise f'The type of in_features {type(out_features)} is not' 'supported, Only int or MutableValue is supported currently.'
    ...
    def fix_chosen(self) -> None:
        ...
        in_features = (
            self.in_features.current_value if isinstance(
                self.in_features, MutableValue) else self.in_features)
        out_features = (
            self.out_features.current_value if isinstance(
