...
class RandomSearcher(object):
    def __init__(self, trainer: NB201Trainer = None, model_path=None):
        ...
        if model_path is None:
            model_path = (
                './checkpoints/test_nb201_spos/test_nb201_spos_macro_ckpt_0191.pth.tar'
            )
        state_dict = torch.load(model_path)['state_dict']
        self.model.load_state_dict(state_dict)

    def search(self, epochs=20, sample_per_epoch=3, train_loader=None, val_loader=None):
        best_top1_err = 100
        best_subnet = None
    ...
    def draw_random_process(self):
        splited_recoder = [
            [self.recorder[j][i] for j in range(len(self.recorder))] for i in range(3)
        ]
