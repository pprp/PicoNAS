...
def thresholdize(array, divisions=[99, 97, 95, 90, 80]):
    ...
    array = [
        7
        if acc > thresholds_map[7]
        else 6
        if acc > thresholds_map[6]
        else 5
        if acc > thresholds_map[5]
        else 4
        if acc > thresholds_map[4]
        else 3
        if acc > thresholds_map[3]
        else 2
        if acc > thresholds_map[2]
        else 1
        if acc > thresholds_map[1]
        else 0
        for acc in array
    ]
    # array = [5 if acc > thresholds_map[4] else 4 if acc > thresholds_map[3] else 3 if acc > thresholds_map[2] else 2 if acc > thresholds_map[1] else 1 if acc > thresholds_map[0] else 0 for acc in array]
...
def stackize(dataframe, preserve_names):
    for index, row in tqdm(dataframe.iterrows()):
        try:
            ...
            dataframe.at[index, preserve_names] = (
                row[preserve_names].detach().numpy().tolist()
            )
...
if not skip_adj:
    ...
    if subsample_adj is None:
        for i in tqdm(range(len(nb101.nb1_api.hash_iterator()))):
            ...
            acc_list.append(
                nb101.nb1_api.get_metrics_from_hash(hash_iterator[i])[1][108][0][
...
if skip_adj:
    master_dict = {
        'nb101': {'cate': cate_nb101, 'zcp': zcp_nb101, 'arch2vec': arch2vec_nb101}
    }
    master_dict['nb201'] = {
        'cate': cate_nb201,
        'zcp': zcp_nb201,
...
for search_space in master_dict.keys():
    for method in master_dict[search_space].keys():
        ...
        sns.scatterplot(
            ...
            data=df,
        ).set(title=f'{search_space} {method} T-SNE projection')
