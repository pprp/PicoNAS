...
def main(args):
    ...
    cam_extractors = [
        methods.__dict__[name](model, enable_hooks=False) for name in cam_methods
    ]
    ...
    for idx, extractor in zip(range(1, len(cam_extractors) + 1), cam_extractors):
        ...
        ax = (
            axes[idx // num_cols][idx % num_cols]
            if args.rows > 1
            else axes[idx]
            if num_cols > 1
            else axes
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--arch', type=str, default='resnet18', help='Name of the architecture'
    )
    parser.add_argument(
        '--img',
        type=str,
    ...
    parser.add_argument(
        '--class-idx', type=int, default=232, help='Index of the class to inspect'
    )
    parser.add_argument(
        '--device',
        type=str,
