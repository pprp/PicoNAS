...
from .net_ops.cell_ops import ConvLayer, DeconvLayer
...
class SegmentationDecoder(nn.Module):
    def __init__(self, in_dim, target_dim, target_num_channel=3, norm=nn.BatchNorm2d):
        ...
        model = [ConvLayer(in_channel, 1024, 3, 1, 1,
        ...
        for i in range(6 - num_upsample):  # add non-upsampling layers
            model += [
                ConvLayer(
                    tmp_in_C, tmp_in_C // 2, 3, 1, 1, nn.LeakyReLU(
                        0.2, False), norm
                )
            ]
            tmp_in_C = tmp_in_C // 2
        ...
        for i in range(num_upsample - 1):  # add upsampling layers
            model += [
                DeconvLayer(
                    tmp_in_C, tmp_in_C // 2, 3, 2, 1, nn.LeakyReLU(
                        0.2, False), norm
                )
            ]
            tmp_in_C = tmp_in_C // 2
        ...
        model += [
            DeconvLayer(tmp_in_C, tmp_in_C, 3, 2, 1,
                        nn.LeakyReLU(0.2, False), norm)
        ]
