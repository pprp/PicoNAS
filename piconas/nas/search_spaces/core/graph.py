...
from piconas.utils.pico_logging import log_first_n, log_formats
from piconas.utils.utils import AttrDict, iter_flatten
    ...
    **Interface to tabular benchmarks**
    ...
    def get_all_edge_data(self,
        ...
        Args:
            ...
            private_edge_data (bool): Whether to return data from graph
            copies as well.
    ...
    def compile(self):
        """
        Instanciates the ops at the edges using the arguments specified at
        the edges
        """
    ...
    def _verify_update_function(update_func: callable,
        ...
        if private_edge_data:
            assert result._shared == test._shared, (
                'The update function changes shared data although '
    ...
    def update_edges(self,
        ...
        Args:
            update_func (callable): Function which accepts one argument
                called `current_edge_data`.
    ...
    def update_nodes(self,
        ...
        This is the preferred way to change the search space once it
        has been defined.

        Note that edges marked as 'final' will not be updated here.

        Args:
            update_func (callable): Function that accepts three incoming
                parameters named `node, in_edges, out_edges`.
            ...
            scope (str or list(str)): Can be "all" or list of scopes to
                be updated. Only graphs and child graphs with the specified
                scope are considered
            single_instance (bool): If set to false, this means update_func
                will be applied to nodes of all copies of a graphs. THIS IS
                NOT RECOMMENDED FOR SHARED ATTRIBUTES, i.e. when manipulating
                the shared data of incoming or outgoing edges. Shared attributes
                should be set only once, we take care it is syncronized across
