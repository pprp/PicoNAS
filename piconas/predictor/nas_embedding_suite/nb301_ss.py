...
class NASBench301:
    ...
    def __init__(self,
                 use_nb3_performance_model=False,
                 zcp_dict=False,
                 normalize_zcp=True,
                 log_synflow=True,
                 path=None,
        ...
        self.cate_nb301 = torch.load(BASE_PATH +
                                     'cate_embeddings/cate_nasbench301.pt')
        self.nb301_proxy_cate = None
        self.arch2vec_nb301 = torch.load(
            BASE_PATH +
            'arch2vec_embeddings/arch2vec-model-dim_32_search_space_nasbench301-nasbench301.pt'
        )

        self.nb3_api = nb3_api
        if self.use_nb3_performance_model:
            self.performance_model = nb3_api.load_ensemble(
                self.ensemble_dir_performance)
    ...
    def get_adjmlp_zcp(self, idx):
        ...
        return np.concatenate([adj_mat, op_mat,
    ...
    def get_adj_op(self, idx, space=None, bin_space=None):
        ...
        return convert_arch_tuple_to_idx([(str(x[0]), self.op_dict[x[1]])
                                          for x in arch_desc['arch'][0]])
    ...
    def get_valacc(self, idx, space=None, use_nb3_performance_model=False):
        if use_nb3_performance_model:
            ...
            return self.performance_model.predict(
                config=cate_nb301_arch,
                representation='genotype',
                with_noise=self.noisy_nb3)
        else:
