    ...
    def __init__(self):
        ...
        for space, space_idx in self.ss_mapper.items():
            if space in ['nb101', 'nb201', 'nb301', 'tb101']:
                exec(
                    f'self.ss_mapper_oprange[space] = (np.asarray(self.{space}.get_adj_op(0)["module_operations"]).shape[-1], space_idx)'
                )
            else:
                exec(
                    f'self.ss_mapper_oprange[space] = (np.asarray(self.nds.get_adj_op(0, space="{space}")["normal_ops"]).shape[-1], space_idx)'
                )
