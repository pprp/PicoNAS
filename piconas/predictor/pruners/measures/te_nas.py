...
def compute_RN_score(
    model: nn.Module, batch_size=None, image_size=None, num_batch=None, gpu=None
):
    # # just debug
    # gpu = 0
    # import ModelLoader
...
@measure('tenas', bn=True)
def compute_tenas(net, inputs, targets, split_data=1, loss_fn=None):
    ntk_score = get_ntk_n(
        [net],
        recalbn=0,
        train_mode=True,
